...
Module.register("calendar_monthly", {
  ...
  start: function () {
    ...
    var now = moment();
    this.midnight = moment([now.year(), now.month(), now.date() + 1]).add(
  ...
  getDom: function () {
    if (moment() > this.midnight || !this.loaded) {
      ...
      if (this.config.debugging) {
        footerTD.innerHTML =
          "Calendar currently in DEBUG mode!<br />Please see console log.";
      } else {
        footerTD.innerHTML = "&nbsp;";
      }
      ...
      for (var i = 0; i < 9; i++) {
        ...
        for (var j = 0; j <= 6; j++) {
          ...
          if (j < startingDay && i == 0) {
            ...
            innerSpan.innerHTML = moment()
              .subtract(1, "months")
              .endOf("month")
              .subtract(startingDay - 1 - j, "days")
              .date();
            ...
            innerSpan.innerHTML = moment([year, month, monthLength])
              .add(nextMonth, "days")
              .date();
            nextMonth++;
          }
  ...
  scheduleUpdate: function (delay) {
    ...
    if (delay > 0) {
      ...
      if (this.config.debugging) {
        ...
        var secs =
          Math.floor(nextReload.asSeconds()) - (hours * 3600 + mins * 60);
        Log.log(
          "  nextReload should happen at: " +
            delay +
            " (" +
            moment(delay).format("hh:mm:ss a") +
            ")"
        );
        Log.log(
          "                  which is in: " +
        ...
        Log.log(
          "              midnight set at: " +
            this.midnight +
            " (" +
            moment(this.midnight).format("hh:mm:ss a") +
            ")"
        );
        Log.log("= = = = = = = = = = = = = = = = = = = = = = = = = = = = = =");
      }
